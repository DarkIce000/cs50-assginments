...
def index():
    ...
    portfolio = db.execute(
        "SELECT symbols, SUM(shares) AS total_shares FROM transactions WHERE user_id = ? GROUP BY symbols",
        username(),
    )
    data_list = []
    stock_price = 0.00
...
def buy():
    if request.method == "POST":
        ...
        if not username() == None:
            ...
            db.execute(
                "INSERT INTO transactions (user_id, transaction_type, symbols, shares, price, transaction_time) VALUES (?, ?, ?, ?, ?, ?)",
                username(),
                "buy",
                data["name"],
                share,
            ...
            db.execute(
                "UPDATE users SET cash = ? where username = ?", update_cash, username()
            )
...
def history():
    ...
    data = db.execute(
        "SELECT symbols, shares, price, transaction_time FROM transactions WHERE user_id = ? ORDER BY transaction_time DESC",
        username(),
    )
...
def quote():
    if request.method == "POST":
        ...
        if data is None:
            return apology("invalid")
        return render_template(
            "quoted.html", Name=data["name"], Symbol=data["symbol"], Price=data["price"]
        )
    else:
...
def sell():
    ...
    if request.method == "POST":
        ...
        sShares = db.execute(
            "SELECT SUM(shares) AS sumsShares FROM transactions where user_id = ? AND symbols = ? AND transaction_type = 'sell'",
            username(),
            symbol,
        )
        bShares = db.execute(
            "SELECT SUM(shares) AS sumsShares FROM transactions where user_id = ? AND symbols = ? AND transaction_type = 'buy'",
            username(),
            symbol,
        )
        ...
        db_symbols = db.execute(
            "SELECT symbols FROM transactions where user_id = ? GROUP BY symbols",
            username(),
        )
